...
void __fastcall hooks::hooked_runcommand::hook(void* ecx, void* edx, player_t* player, CUserCmd* m_pcmd, IMoveHelper* move_helper)
{
	//static auto o_runcommand = prediction_hook->get_func_address <RunCommand_t>(19);
	g_ctx.local((player_t*)m_entitylist()->GetClientEntity(m_engine()->GetLocalPlayer()), true);

	if (!player || player != g_ctx.local())
		return o_runcommand(ecx, player, m_pcmd, move_helper);
	...
	if (g_cfg.ragebot.enable && player->is_alive())
	{
		...
		o_runcommand(ecx, player, m_pcmd, move_helper);
...
bool __fastcall hooks::hooked_writeusercmddeltatobuffer::hook(void* ecx, void* edx, int slot, bf_write* buf, int from, int to, bool is_new_command) 
{
	...
	//static auto o_writeusser = client_hook->get_func_address <WriteUsercmdDeltaToBuffer_t>(24);

	if (!g_ctx.globals.tickbase_shift)
		return o_writeusser(ecx, slot, buf, from, to, is_new_command);
	...
	{
		while (o_writeusser(ecx, slot, buf, from, to, true))
		{
			from = to++;
