...
bool __stdcall hooks::hooked_createmove::hook(float smt, CUserCmd* m_pcmd)
{
	static auto original_fn = o_createmove;
	g_ctx.local((player_t*)m_entitylist()->GetClientEntity(m_engine()->GetLocalPlayer()), true);
		
	g_ctx.globals.in_createmove = false;

	if (!m_pcmd)
		return original_fn(smt, m_pcmd);

	if (!m_pcmd->m_command_number)
		return original_fn(smt, m_pcmd);

	auto original_result = original_fn(smt, m_pcmd);
